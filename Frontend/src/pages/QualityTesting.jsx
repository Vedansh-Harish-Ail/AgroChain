...
export default function QualityTesting() {
  ...
  const [physicallyVisited, setPhysicallyVisited] = useState(false);
              ...
              <div className="space-y-4 text-sm">
                ...
                {user?.role !== 'TESTER' ? (
                  <div className="space-y-3">
                    <div className="pt-2">
                      <label className="flex items-start gap-3 cursor-pointer p-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-800 rounded-2xl hover:bg-slate-100/50 dark:hover:bg-slate-800/80 transition-colors">
                        <input
                          type="checkbox"
                          checked={physicallyVisited}
                          onChange={(e) => {
                            setPhysicallyVisited(e.target.checked);
                            setInspectionMethod(e.target.checked ? 'PHYSICAL_VISIT' : 'PHOTO_REVIEW');
                          }}
                          className="mt-0.5 h-4 w-4 rounded border-slate-350 text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                        />
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-350 select-none leading-relaxed">
                          I have physically visited the farm and verified the crop presence and quality parameters.
                        </span>
                      </label>
                    </div>
               ...
               {user?.role === 'TESTER' ? (
                ...
                <div className="space-y-3 pt-2">
                  <button
                    ...
                    disabled={loading || savingNotes || isActionDisabled || !physicallyVisited}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      disabled={loading || savingNotes || isActionDisabled || !physicallyVisited}
                      className="flex justify-center items-center gap-1 py-2.5 rounded-xl border border-rose-200 text-rose-600 hover:bg-rose-50 dark:border-rose-955/30 dark:hover:bg-rose-950/20 text-xs font-bold transition disabled:opacity-50"
                    >
